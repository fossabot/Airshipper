...
pub(crate) async fn update() -> Result<()> {
    ...
    if let Some(url) = network::check_win_update().await.ok().flatten() {
        ...
        if crate::cli::confirm_action()? {
            ...
            let filename = match url::Url::parse(&url)?.path_segments().map(|x| x.last()).flatten() {
            ...
            if resp.status().is_success() {
                ...
                log::debug!("Download airshipper update to: {}", path.join(&filename).display());
                ...
                if !(result as c_int > 32) {
                    log::error!("Failed to update airshipper! {}", std::io::Error::last_os_error());
                }
                std::process::exit(0);
            }
        }
